...
所有依赖路径相对于清单目录，必须显式声明。默认 `model_visible=[]`，不向模型提供任何依赖文件；默认 `feedback_policy="category_only"`，只反馈错误类别。只有确认测试诊断允许公开时才使用 `public_diagnostics`。可省略 testbench，此时显式清单提供的顶层和依赖只用于综合，功能仍标为未验证。
