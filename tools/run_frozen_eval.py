...
VITIS = Path('/home/dingjy/sxt/zcomp-agent/vivado/2025.2/Vitis')
    ...
    with log.open('wb') as output:
        ...
        try:
            ...
            try:
                ...
                pass
            proc.wait()
    return dict(exit_code=proc.returncode, timed_out=timed_out, seconds=round(time.monotonic()-started, 3), log=str(log.relative_to(ROOT)))
...
def setup_lines(task, work):
    ...
    lines += ['open_solution -flow_target vivado solution1', 'set_part ' + MANIFEST['part'], 'create_clock -period 5 -name default']
    return lines
...
def stage_tcl(task, work, env, stage, command, timeout):
    ...
    result = execute([str(VITIS / 'bin/vitis-run'), '--mode', 'hls', '--tcl', str(script)], work, env, work / (stage + '.log'), timeout)
    ...
    result['diagnostics'] = '\n'.join(l for l in text.splitlines() if re.search(r'error:|ERROR:|fatal|mismatch', l, re.I))[-5000:]
    return result
...
def env_for(work):
    ...
    env.update(XILINX_VITIS=str(VITIS), XILINX_HLS=str(VITIS), XILINX_VIVADO=str(VIVADO),
               XILINXD_LICENSE_FILE='/home/dingjy/sxt/zcomp-agent/vivado/vivado_license.lic',
               OMP_NUM_THREADS='1', OPENBLAS_NUM_THREADS='1')
    libs = [VITIS / 'lib/lnx64.o', VITIS / 'lnx64/lib/csim', VITIS / 'lnx64/tools/gcc/lib64', VITIS / 'lnx64/tools/clang-16/lib']
...
def evaluate(task):
    ...
    started = time.monotonic()
    result = dict(task_id=task['task_id'], status='running', checks={s:'not_run' for s in ['parse', 'compile', 'run', 'synthesize']},
...
def main():
    ...
    tasks = [t for t in MANIFEST['tasks'] if args.task is None or t['task_id'] == args.task]
    summary = dict(status='running', task_count=len(tasks), completed=0, results=[],
                   note='Development evaluation using Vitis 2025.2; parsing inferred from successful compilation, not independent official parsing score. Exact reference testbench output comparison; no hidden tests.',
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            print(json.dumps(dict(task=r['task_id'],status=r['status'],checks=r['checks'], completed=summary['completed'],total=len(tasks))), flush=True)
    summary['status'] = 'completed'
    summary['finished_at'] = datetime.now(timezone.utc).isoformat()
    summary['counts'] = {s:sum(r['checks'][s]=='passed' for r in summary['results']) for s in ['parse','compile','run','synthesize']}
    write(summary_file, summary)
