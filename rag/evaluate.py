...
def main():
    ...
    a = p.parse_args()
    if a.top_k < 1 or a.recall_k < a.top_k or a.max_bytes < 1:
    ...
    titles = {r['title'].lower() for r in retriever.records}
    for q in data['queries']:
        if not any(t.lower() in titles for t in q['titles']):
    ...
    for mode in a.modes:
        ...
        for i, q in enumerate(data['queries']):
            ...
            gold = {t.lower() for t in q['titles']}
            flags = [h['record']['title'].lower() in gold for h in result['hits']]
