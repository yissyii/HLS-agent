...
- [x] 在 diagnostic_v1 中加入错误签名门；重排只能改变通过门的候选顺序，不能绕过适用性拒绝。
- [x] 记录 eligible/rejected 数量和 `injection_policy=eligible_only`；候选不足时不为凑满 Top-k 强行注入。

词项门是保守工程启发式，可能错拒同义表达，也可能接受表面重合；不是已校准的语义相关性判断。它必须记录通过／拒绝原因，并通过后续人工标注进一步校准。当前 `diagnostic_v1` 在词项门之后增加签名门：接口 offset 必须同时出现合法模式，`ap_int/ap_uint` 调用错误必须命中位选择／范围选择证据，纯 C++ 的未声明标识符、缺分号、重复定义、非法字面量和数组初始化错误默认 abstain，缺失头文件必须有对应头文件修复证据。关闭 RAG 的请求保持不变，首稿与 system prompt 不变。检索仍只使用题目与当前验证器释放的反馈，不读原始日志、源码或隐藏测试。
...
查询清理编译器文件位置、时间戳及重复行，将 error 行优先，题目最多占 256 字符且不超过可用查询长度的四分之一。候选在 Top-k 截选之前经过词项门：至少命中两个诊断有效词；出现明确的 missing member 时还必须包含该成员名。随后执行错误签名门：纯 C++ 语法错误默认拒绝，`ap_int/ap_uint` 需出现位选择／范围选择证据，接口 offset 需出现完整合法模式。该门只证明词项和签名对应，**不证明条目提供有效修复**。完整原文保持不变，放不下的小预算会导致不注入，而非截断正文。

`query_plan` 记录策略、诊断词、必要成员名和识别出的错误签名；`selection_audit` 记录候选的词项匹配、签名门和通过／拒绝原因。`accepted` 表示候选同时通过适用性门，实际模型是否看到该条仍以 `injected_ids` 为准。`Retriever` 返回 `eligible_count`、`rejected_count`、`top_k_requested` 和 `injection_policy=eligible_only`；低于 Top-k 的可用候选不会用低质量条目补齐。低信息反馈记录 `status=skipped` 和原因，不启动检索 worker；合法无命中继续修复。真正的超时／发布校验失败仍明确报错。
