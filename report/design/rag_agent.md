...
默认每路召回 20 条、最终最多 3 条、参考区含包装不超过 6000 UTF-8 字节；查询最多 1600 字符。每次检索最多 30 秒，并扣除剩余总时间与验证预留。Qwen 运行在本地 CPU，worker 每次修复重新加载；后续可用常驻服务减少冷启动，但本版优先保证独立进程可超时终止。未增加模型重排器，混合排序仍是 RRF。
