...
更新日期：2026-09-18。本文按当前源码核对；历史测试记录保留原测试日期，后续计划集中放在最后一节。

**一句话理解：Agent 让模型生成 HLS C++，用 Vitis 检查结果，再根据报错决定是否修复，最后交付验证证据最好的候选代码。**

阅读建议：先看第 1～3 节理解整体；开发模块时看第 4～6 节；查看运行结果时看第 7 节。命令和参数示例见 [使用说明](../../agent/README.md)。
...
| `serve/code.py` | 从原文或 Markdown 中提取源码，按固定结构规则选择最可能完整的代码块 |
| `evaluation/task_io.py` | 读取题目、清单和明确声明的依赖 |
| `evaluation/validator.py` | 为每个候选准备独立工作目录，整理验证证据 |
| `evaluation/hls.py` | 启动 Vitis，执行仿真、综合及超时处理 |
| `evaluation/diagnostics.py` | 把 Vitis 日志解析为带类型（compiler/synthesis/functional 等）的结构化诊断条目 |
...
| `feedback_policy` | `category_only` | 反馈边界：`category_only` 只给错误类别；`compiler_diagnostics` 给编译/综合具体错误（源码行 + note）；`public_diagnostics` 给原始日志诊断 |
...
| `ValidationResult` | 被验证的代码/任务/工具配置哈希、阶段结果及按 `feedback_policy` 放行的反馈文本 |
...
| 返回多个代码块 | 按结构完整性、顶层函数和测试台特征等固定规则选择一个块；同分选最先出现的块 |
| 没有可用源码块，或生成因 token 上限等原因截断 | 记录失败，不拼接、不自动续写 |
| 所需验证全部通过 | 立即停止 |
...
诊断文本来自 `evaluation/diagnostics.py` 的结构化提取：`compiler_diagnostics` 档放行编译与综合条目（含源码行、脱字符和去重后的 note），功能测试的 `Mismatch` 反例与环境/许可证噪声只在 `public_diagnostics` 档才进入提示词。
...
baseline 和 Agent 共用 `serve/code.py`，采用 `ranked_fences_v1` 规则。纯源码保持原文；Markdown 响应允许代码块前后有说明，并兼容反引号、波浪线围栏及常见 C/C++ 语言标签。

多个代码块不会直接判为格式失败。先排除未闭合、空块和明确标为其他语言的块；无语言标签的块需要有函数、头文件或常见 C/C++ 类型等源码信号。对剩余代码块，按以下顺序比较：

1. 优先存在函数定义且括号、注释、字符串在结构检查中闭合的实现，其次比较结构是否闭合。
2. 已知 `top_function` 时，优先实际定义该函数的块；声明或调用不算定义。
3. 优先不含 `main()` 的设计代码，再比较非 `main` 函数数量、头文件依赖、HLS pragma 和有效代码 token 数。
4. 同分时选响应中最先出现的代码块，记录并列情况。

这是固定的结构启发式，并非完整 C++ 解析器，也不能证明源码可编译或功能正确。注释、字符串里的括号和伪函数不参与判断，增加说明文字不会提高评分。只交付选中的一个块，保留其源码内容；不拼接不同文件、不修补代码，也不通过逐个编译来挑选答案。

Agent 的顶层函数提示来自已显式提供的任务清单；严格裸基线继续只使用模型原始回复，不额外读取任务清单或参考实现。后续编译、功能测试与综合决定候选是否正确。

选择记录包括规则版本、各块特征和评分、选中块编号（从 0 开始）、并列块及裁决方式。baseline 将其写入 `result.json.source_extraction_details`；Agent 写入每个候选的 `extraction.json` 和候选创建事件。原始回复仍保存在 `response.txt`。

模型请求的 `finish_reason` 完整性检查发生在提取之前。`generation_incomplete` 仍是失败，不能因为截断回复里恰好有一个闭合代码块就放行。已正常结束的回复中，如果后面另有未闭合围栏，可选择前面已闭合的有效源码块。

此规则替代 9-17 评测记录所使用的“整个响应恰好一个代码块”规则。历史结果保留原口径，新增通过数需要单独评测，不能把全部格式失败或截断样本直接改计为通过。2026-09-18 已按该规则重新独立采样并完成 HLS 分级评测，官方数字见 [9-18 HLS pass@5 冻结](../evaluations/9-18-HLS-pass5-ranked-fences.md)。
...
└── candidates/
    ...
    │   ├── extraction.json               # 各源码块评分、选择结果及并列裁决
...
已有裸基线与普通修复 Agent 的开发评测，见 [评测索引](../README.md#baseline--agent-评测)。后续独立开发实验继续使用以下对照：
...
项目已有题集按 [基线协议](../reproducibility/baseline_protocol.md) 冻结。开发新策略和技能时，应使用另建的独立开发题，不能依据冻结题的失败结果反复调整方案。

## 9. 当前验证结果与下一步

### 研发评测的必经外层

研发期间，所有现有评测命令先进入 `evaluation/lifecycle.py`，由独立的 `local_eval/` 模块管理完整评测会话。这个外层与具体题库无关，也不需要额外命令或开关。

```text
评测命令 → 配置快照与输入检查 → 完整一轮评测
                                  ↓
                    无网络故障 → 接受本轮结果
                    有网络故障 → 作废整轮 → 等待 → 从头重跑
```

单题命令重跑整道题；批量命令重跑本次选中的整批题；配对命令重跑 baseline 和 agent 两条分支。子任务通过继承的轮次信息上报请求状态，由最外层统一决定重跑。故障后不再发出新的模型请求，已启动的子任务按原有超时收尾，再启动下一轮。

默认识别 HTTP 408、429、500、502、503、504 和底层连接/网络超时，最多额外重跑两次。参数集中于 `local_eval/retry.json`。编译失败、功能错误、输出截断和工具超时仍是正常评测结果，不触发此机制。每轮 Agent 内部的模型调用与修复预算保持原定义。

每轮使用独立输出目录；`session.json` 的 `valid_attempt` 决定哪一轮可用于统计。作废轮日志和调用成本保留，不能混入最终成绩。本文前面的单轮输出目录，在研发会话中位于 `attempt_NNN/result/`（旧自动命名入口位于 `attempt_NNN/artifacts/`）。

提交副本删除整个 `local_eval/`，统一接入层便恢复单轮执行和原输出路径。后续新增评测入口必须接入 `evaluation.lifecycle.evaluate()`；底层单轮执行函数继续服务于控制器和合同测试。使用、字段和集成示例见 [研发评测模块说明](../../local_eval/README.md)。
...
以上结果来自首版实现时的验证，详细记录见 [实现与验证报告](agent_implementation.md)。它们证明工程链路可工作，尚不证明真实模型的比赛收益。

另有 2026-09-16～17 的 Bench4HLS 真实模型开发评测，分别保留旧提取规则、沙箱复跑及提取更新后的结果。后者记录 baseline 89/170、Agent 94/170；两条入口独立采样，不能把差异全部归因于修复。2026-09-18 已完成独立 UG1399 混合检索实测，但 RAG 尚未接入本控制器。报告与限制见 [总索引](../README.md)。
