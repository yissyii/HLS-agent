...
研发环境的 [评测管理模块](../../local_eval/README.md) 自 2026-09-23 永久改为仅重试外部故障请求，保留有效结果；不再整轮作废。baseline 仍只有一次有效生成机会，传输重试单独计数。未恢复外部故障单列，不进入能力失败分母；通过率同时报告覆盖率。显式输出目录通过 `session.json.valid_attempt` 定位产物。从提交副本移除 `local_eval/` 后恢复单次请求行为。

2026-09-17 起，提取规则允许 Markdown 代码块前后有解释文字。多个 C/C++ 或具有源码特征的无标签代码块，按结构闭合、函数定义、测试台特征及支持代码等固定规则排序，选取最可能完整的一个；同分选最先出现者。裸基线不额外读取任务清单来选择代码。评分只看响应结构，不编译候选、不拼接代码、不增加模型请求；细节见 [设计文档的响应提取说明](../design/design.md#64-响应提取与多代码块选择)。
...
当前官方 **HLS pass@5 冻结**（2026-09-18）：122 题 × 5 次 = 610 槽位，提取规则 `ranked_fences_v1`，模型 `qwen38`，温度 0.7，`max_tokens` 4096，thinking 关闭。网络/HTTP 失败已重跑至非网络结果，不计入模型失败。RTL 未评估。完整口径、对比与问题见 [9-18 HLS pass@5 冻结](../evaluations/9-18-HLS-pass5-ranked-fences.md)。
...
最终状态守恒：未形成 133（格式 103 + 截断 30）+ 编译失败 228 + 运行失败 46 + 输出不匹配 41 + 综合失败 6 + 完整通过 156 = 610。可验证候选 477。原始批次 `output/baseline_pass5_ranked_fences_v1_20260918/`，Vitis 验证包 `output/baseline_pass5_ranked_fences_v1_20260918_validation/`。

相对 2026-09-16（旧提取、同样超参）：可验证候选 395→477；可综合 pass@1 20.82%→25.57%，pass@5 34.43%→36.89%。候选内部综合通过率几乎不变（32.2%→32.7%）；温度 0.7，不能把差值全部归因于提取规则。09-16 数字保留为历史对照，不以本轮覆盖改写。

`tools/preserve_baseline_evidence.py` 将更早的历史生成批次、重试记录和已下载的评测包存为独立压缩包，并逐文件校验 SHA256，原文件保留。索引明确标记为历史开发结果：输入曾含包装／头文件、存在重试及单题提示干预，且没有同期智能体结果，不能改标为赛事正式基线。

该历史批次累计保存 121/122 份生成代码；独立评测为解析 107、编译 106、公开测试校验 59、综合 49。综合另有 4 项明确错误、6 项固定预算超时；1 项参考校验异常。详细结果位于 `output/frozen_eval_121_20260915/remote_results/audited_summary.json`。不以该统计冒充官方 pass@1/pass@5。
