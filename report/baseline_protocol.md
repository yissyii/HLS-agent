...
当前官方 **HLS pass@5 冻结**（2026-09-18）：122 题 × 5 次 = 610 槽位，提取规则 `ranked_fences_v1`，模型 `qwen38`，温度 0.7，`max_tokens` 4096，thinking 关闭。网络/HTTP 失败已重跑至非网络结果，不计入模型失败。RTL 未评估。完整口径、对比与问题见 [record/9-18-HLS-pass5-ranked-fences.md](../record/9-18-HLS-pass5-ranked-fences.md)。
